...
********************************************************************************
		...
		*The if are applying due to changes in the IV
				  ...
				  collect _r_b _r_se, tag(model[(`number')]): selmlog HDDS`number' ///
				  $foodsecurity_eq, select(combined_fisp_saps =  `selection_`number'') boot(300) dmf(0) gen(m`number')
...
foreach y of local outcomes {
    ...
    * Outcome label for filenames/titles
    ...
    format actual_treated cf_treated_none ATT cf_untreated_treated actual_untreated ///
           ATU BH1 BH2 TH %9.2f
    format pct_change %9.2f
    format p_ATT p_ATU %9.2f
    ...
    gen str15 ATT_disp = string(ATT,"%9.2f") + ATT_star
    gen str15 ATU_disp = string(ATU,"%9.2f") + ATU_star
    gen str12 pct_disp = string(round(pct_change,0.1),"%9.2f")
